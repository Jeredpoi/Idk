...
internal static class Cues
{
    ...
    private static void Play(SoundPlayer player)
    {
        ...
        // ⚠️ ЗВУК УХОДИТ С ПОТОКА ПЕРЕХВАТЧИКА, И ЭТО НЕ ОПТИМИЗАЦИЯ. Зовут нас в том числе из
        // колбэка хука — то есть с потока, который ждёт Windows и снимает перехватчик, если тот
        // задумался дольше 300 мс. Обращение к звуковой подсистеме такой гарантии не даёт вообще:
        // первый вызов будит устройство, а на некоторых картах это десятки миллисекунд и больше.
        // Плюс исключение, вышедшее из колбэка в неуправляемый код, завершает процесс молча.
        // Отдельный поток снимает оба риска сразу.
        ThreadPool.QueueUserWorkItem(_ =>
        {
            ...
            {
                // Список типов здесь был бы гаданием: winmm отвечает по-разному на отсутствие
                // устройства, занятый сеанс и урезанные политики. Молчание — приемлемый исход
                // для звуковой метки; падение из-за неё — нет.
