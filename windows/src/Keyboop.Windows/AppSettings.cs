...
public sealed class AppSettings
{
    ...
    /// <summary>
    /// Короткие звуковые метки: исправленная раскладка, начало и конец диктовки.
    ///
    /// Включено по умолчанию — как в macOS-версии. Метка тихая и короткая, а без неё непонятно,
    /// сработала программа или нет: значок в трее во время набора никто не видит. Выключается
    /// одним пунктом меню, и этот пункт гасит ВЕСЬ звук, который издаёт программа.
    /// </summary>
    public bool Sounds { get; set; } = true;

    /// <summary>
    /// Caps Lock мгновенно переключает язык вместо включения верхнего регистра.
    ///
    /// Выключено по умолчанию: менять поведение системной клавиши без спроса нельзя, даже если
    /// нам кажется, что так лучше.
    /// </summary>
    public bool CapsSwitchesLayout { get; set; }
